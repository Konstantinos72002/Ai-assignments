...
def sentence3() -> Expr:
    ...
    return conjoin(alive,both_alive,born)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for (x, y) in all_coords:
        wallpos = PropSymbolExpr(wall_str, x, y)
        pacpos = PropSymbolExpr(pacman_str, x, y, time=t)
        list.append(wallpos >> ~pacpos)
    pacphysics_sentences.append(conjoin(list))

    list: List[Expr] = []
    for (x, y) in non_outer_wall_coords:
        list.append(PropSymbolExpr(pacman_str, x, y, time=t))
    pacphysics_sentences.append(exactlyOne(list))

    list: List[Expr] = []
    for action in DIRECTIONS:
        list.append(PropSymbolExpr(action, time=t))
    pacphysics_sentences.append(exactlyOne(list))
    ...
    util.raiseNotDefined(exactlyOne(list))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    next_pos = PropSymbolExpr(pacman_str, x1, y1, time=1)
    model1 = findModel(conjoin(KB) & next_pos)
    model2 = findModel(conjoin(KB) & ~next_pos)
...
def positionLogicPlan(problem) -> List:
    ...
    # Insert start location to KB
    start_location = PropSymbolExpr(pacman_str, x0, y0, time=0)
    KB.append(start_location)
    for t in range(50):
        print(t)
        
        # Insert for 50 times the expresion of pacman can be exist at only one legal location
        posLoc: List[Expr] = []
        for (x, y) in non_wall_coords:
            posLoc.append(PropSymbolExpr(pacman_str, x, y, time=t))
        KB.append(exactlyOne(posLoc))
        
        # Insert for 50 times the expresion of pacman can make only one legal action
        acts: List[Expr] = []
        for act in actions:
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for (x, y) in food:
            ...
            # if pacman is in foodpos, food in foodpos next time have be eaten 
            KB.append((foodpos & pacpos) >> ~foodpos1)
            # if pacman is'nt in food pos, food stay at foodpos at next time
            KB.append(~pacpos >> (foodpos1 % foodpos))
        if t > 0:
            for (x, y) in non_wall_coords:
                KB.append(pacmanSuccessorAxiomSingle(x, y, time=t, walls_grid=walls))

        # if all food isn't at their spot you are in goal state
        goallist: List[Expr] = []
        for (x, y) in food:
            goallist.append(~PropSymbolExpr(food_str, x, y, time=t))
        goal = conjoin(goallist)
        model = findModel(conjoin(KB) & goal)
        if model:
            plan = extractActionSequence(model, actions)
            return plan
